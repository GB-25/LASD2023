...
public:
  ...
  // Copy assignment
  virtual Queue& operator=(const Queue&)=delete; // Copy assignment of abstract types should not be possible.

  // Move assignment
  virtual Queue& operator=(Queue&&)noexcept =delete; // Move assignment of abstract types should not be possible.
  ...
  virtual Data& Head() const =0; // (mutable version; concrete function must throw std::length_error when empty)
